...
class BooksApiTestCase(APITestCase):
    def setUp(self):
        ...
        self.book_1 = Book.objects.create(name='Test book 1', price=25, author_name='Author 1',
                                          owner=self.user)
        self.book_2 = Book.objects.create(name='Test book 2', price=52, author_name='Author 2',
                                          owner=self.user)
        self.book_3 = Book.objects.create(name='Test book Author 1', price=55,
                                          author_name='Author 3',
                                          owner=self.user)
        UserBookRelations.objects.create(user=self.user, book=self.book_1, like=True,
                                         rate=5)
    ...
    def test_filter(self):
        ...
        response = self.client.get(url, data={'price': 52})
        serializer_data = BooksSerializer(books, many=True).data
        self.assertEqual(status.HTTP_200_OK, response.status_code)
        self.assertEqual(serializer_data, response.data)
...
class BooksRelationApiTestCase(APITestCase):
    def setUp(self):
        ...
        self.book_1 = Book.objects.create(name='Test book 1', price=25, author_name='Author 1',
                                          owner=self.user)
        self.book_2 = Book.objects.create(name='Test book 2', price=52, author_name='Author 3',
                                          owner=self.user)
